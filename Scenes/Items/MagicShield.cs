...
namespace GodotSurvivor.Scenes.Items
{
	public partial class MagicShield : Area2D, IAbility
	{
		...
		private List<Upgrade> CreateUpgrades()
		{
			...
			{
				new("magic_shield.attack_delay", "Attack Delay", "-10% Attack Delay", UpgradeType.Ability, Metadata.Name, () => DelayMultiplier -= 0.1f, targetTexturePath: Metadata.TexturePath, targetCollectibleId: CollectibleId),
				new("magic_shield.damage", "Damage", "+20% Damage", UpgradeType.Ability, Metadata.Name, () => DamageMultiplier += 0.2f, targetTexturePath: Metadata.TexturePath, targetCollectibleId: CollectibleId),
				new("magic_shield.size", "Size", "+15% Size", UpgradeType.Ability, Metadata.Name, () => SizeMultiplier += 0.15f, targetTexturePath: Metadata.TexturePath, targetCollectibleId: CollectibleId),
				new("magic_shield.burning", "Burning", "50% Chance to apply Burning", UpgradeType.Ability, Metadata.Name, () => this.AddApplyableStatus(nameof(Burning), 0.5f, Burning.CreateCustomPackedScene(1, 1f)), null, 1, Metadata.TexturePath, CollectibleId)
			};
		}
	}
}
